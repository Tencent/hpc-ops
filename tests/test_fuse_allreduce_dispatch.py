...
def ref_fuse_allreduce_dispatch_task(input_list, attn_dp_size, attn_tp_size):
    N, H = input_list[0].shape
    device = input_list[0].device
    dtype = input_list[0].dtype

    input_sum = torch.zeros((attn_dp_size * N, H), dtype=dtype, device=device)

    for dp_idx in range(attn_dp_size):
        start = dp_idx * attn_tp_size
        end = start + attn_tp_size
        for tp_idx in range(start, end):
            input_sum[dp_idx * N : (dp_idx + 1) * N, :] += input_list[tp_idx]

    return input_sum


def run_fuse_allreduce_dispatch_task(
    rank,
    world_size,
    local_size,
    N,
    ...
    attn_tp_group_rank = world_rank // attn_tp_size
    ...
    symm_input, in_hdl = hpc.empty_multinode(
        comm, [N_pad, H], dtype=torch.bfloat16, device=device, sub_team=sub_team
    )
    input = input_list[world_rank]
    symm_input[:N, :] = input
    symm_output, out_hdl = hpc.empty_multinode(
        comm, [attn_dp_size * N_pad, H], dtype=torch.bfloat16, device=device
    )
    ...
    rtol = 1e-01
    atol = 1e-01
    ref_output = ref_fuse_allreduce_dispatch_task(input_list, attn_dp_size, attn_tp_size)

    start_idx = N_pad // attn_tp_size * attn_tp_rank
    end_idx = N_pad // attn_tp_size * (attn_tp_rank + 1)

    in_multimem_offset = start_idx * H * symm_input.element_size()
    out_multimem_offset = (attn_tp_group_rank * N + start_idx) * H * symm_output.element_size()

    in_multinode_offset = (
        (local_size // attn_tp_size * node_idx) * N * H * symm_output.element_size()
    )
    out_multinode_offset = (
        (local_size // attn_tp_size * node_idx) * N * H * symm_output.element_size()
    )

    input_slice = symm_input[start_idx:end_idx, :]
    output_slice = symm_output[: local_size // attn_tp_size * N_pad, :]
    output_slice1 = symm_output1[: local_size // attn_tp_size * N_pad, :]

    in_multimem = in_hdl.get_multimem_buff(
        input_slice.shape, dtype=symm_input.dtype, storage_offset=in_multimem_offset
    )
    in_multinode = out_hdl.get_multinode_buff(
        output_slice.shape, dtype=symm_output.dtype, storage_offset=in_multinode_offset
    )
    in_subteam_multimem = in_hdl.get_subteam_multimem_buff(
        input_slice.shape, dtype=symm_input.dtype, storage_offset=in_multimem_offset
    )
    out_multimem = out_hdl.get_multimem_buff(
    ...
    # Two output buffer
    in_multinode1 = out_hdl1.get_multinode_buff(
        output_slice1.shape, dtype=symm_output1.dtype, storage_offset=in_multinode_offset
    )
    out_multimem1 = out_hdl1.get_multimem_buff(
        input_slice.shape, dtype=symm_output1.dtype, storage_offset=out_multimem_offset
    )
    ...
    # (Warning!) When continuously call dispatch kernel, the output buffer should be swapped between two buffers
    # If only use one output buffer, the output buffer may be overwritten by the next dispatch kernel
    out_variants = [
        (
            output_slice,
            out_multimem,
    ...
    # Run "warmup_iters" iterations of accuracy verification, feeding output back as input
    flip = 0
    for verify_iter in range(warmup_iters):
        (
            cur_output_slice,
            cur_out_multimem,
            cur_out_multinode,
        ...
        assert torch.allclose(
            ref_output[: attn_dp_size * N, :],
            cur_symm_output[: attn_dp_size * N, :],
            atol=atol,
            rtol=rtol,
        ), f"Iteration {verify_iter}: " + errors_to_string(
            calculate_errors(
                ref_output[: attn_dp_size * N, :], cur_symm_output[: attn_dp_size * N, :]
            )
        )

        # Use current output as next iteration's input
        if verify_iter < warmup_iters - 1:
            # Update input_list: each rank's new input comes from its dp_group's portion of ref_output
            for i in range(world_size):
                dp_idx = i // attn_tp_size
                input_list[i] = ref_output[dp_idx * N : (dp_idx + 1) * N, :].clone()

            symm_input.zero_()
            symm_input[:N, :] = input_list[world_rank]
            ref_output = ref_fuse_allreduce_dispatch_task(input_list, attn_dp_size, attn_tp_size)
            symm_output.zero_()
            symm_output1.zero_()
            flip = 1 - flip
    ...
    events = [
        ...
        if "fuse_allreduce_dispatch_kernel" in event["name"]
    ]
...
def test_fuse_allreduce_dispatch(
    world_size,
    local_size,
    N,
    H,
    ...
    for rank in range(local_size):
        p = ctx.Process(
            target=run_fuse_allreduce_dispatch_task,
            args=(
                rank,
                world_size,
                local_size,
