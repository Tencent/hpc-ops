...
def make_inputs(
    ...
    inv_freq = 1.0 / (
        ROPE_THETA ** (torch.arange(0, head_dim, 2, dtype=torch.float32, device="cuda") / head_dim)
    )
...
def test_qwen3_tts_rope_rejects_strided_last_dimension():
    ...
    with pytest.raises((RuntimeError, ValueError), match="last dimension must be contiguous"):
        hpc.qwen3_tts_rope(q, k, cos, sin)
...
def _bench_shape(
    ...
    compiled_ref = torch.compile(ref_tts_rope, dynamic=False, options={"epilogue_fusion": False})
    ...
    speedup = triton_us / hpc_us if triton_us == triton_us and hpc_us > 0 else float("nan")
...
def _select_shapes(names: str):
    ...
    if missing:
        raise ValueError(f"unknown shape names: {missing}; available={sorted(TTS_ROPE_CASES)}")
