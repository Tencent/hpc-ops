...
def causal_conv1d_update_ref(zxbcdt, conv_states, weight, bias, indices,
                             num_batch, state_len, conv_dim, d_conv, d_inner,
                             num_head):
  seq_len = 1
  ...
  xbc = xbc.reshape(num_batch, seq_len,
                    conv_dim)  #（n, conv_dim) -> (batch, n, conv_dim)
  xbc = xbc.permute(0, 2, 1).contiguous()  #(batch, conv_dim. seq_len)

  conv_states = conv_states.permute(0, 2, 1).contiguous(
  )  #(max_batch, state_len, conv_dim) -> (max_batch, conv_dim, state_len)
  ...
  weight = weight.permute(
      1, 0).contiguous()  #(d_conv, conv_dim) -> (conv_dim, d_conv)
  ...
  xbc = y.permute(
      0, 2,
      1).contiguous()  #(batch, conv_dim, seq_len) -> (batch, seq_len, conv_dim)
...
def test_mamba_causal_conv1d_update(batch_size, nheads, state_len, d_conv,
                                    d_inner, conv_dim):
  num_max_batch = 256

  conv_states = torch.rand((num_max_batch, state_len, conv_dim),
                           dtype=torch.bfloat16).cuda()
  indices = torch.randperm(num_max_batch)[:batch_size].to(torch.int32).cuda()
  zxbcdt = torch.rand((batch_size, d_inner + conv_dim + nheads),
                      dtype=torch.bfloat16).cuda()
  ...
  hpc.causal_conv1d_update(zxbcdt, conv_states, weight, bias, indices, d_inner,
                           nheads)
  _, xbc, _ = torch.split(zxbcdt, [d_inner, conv_dim, nheads], dim=-1)

  xbc_ref, conv_states_ref = causal_conv1d_update_ref(
      zxbcdt_ref, conv_states_ref, weight_ref, bias, indices, batch_size,
      state_len, conv_dim, d_conv, d_inner, nheads)
