...
def naive_group_gemm_bf16(x, w, cu_seqlens):
    m, k = x.shape
...
def naive_act_mul_bf16(gate_up):
    def silu(x):
        return x / (1 + (-x).exp())
...
def naive_fuse_moe_bf16(
    ...
    # gate_up_proj
    gate_up_output = naive_group_gemm_bf16(gate_up_input, gate_up_weight, cu_seqlens)

    # act_and_mul
    down_input = naive_act_mul_bf16(gate_up_output)

    # down_proj
    down_output = naive_group_gemm_bf16(down_input, down_weight, cu_seqlens)
...
def test_fuse_moe_bf16(
    ...
    x = torch.randn((num_seq, hidden_size), dtype=dtype, device="cuda") / 100
    ...
    if has_shared_output:
        shared_output = torch.randn((num_seq, hidden_size), dtype=dtype, device="cuda")
    else:
        shared_output = None

    my = hpc.fuse_moe_bf16(
        x,
        gate_up_weight,
        down_weight,
        topk_ids,
    ...
    assert allclose(gt.to(torch.float32), my.to(torch.float32), rtol=0.1, atol=0.1)
