...
def run_flash_attention_v2(
    qkvs, batch_size, qo_len, kv_cache_len, num_heads, head_dim, num_kv_heads, v_head_dim, dtype
):
...
def run_flash_attention_v3(
    ...
    try:
        ...
        o = naive_attn_func(
            q.view(batch_size, qo_len, num_heads, head_dim).contiguous(),
            k.view(batch_size, kv_cache_len, num_kv_heads, head_dim).contiguous(),
            v.view(batch_size, kv_cache_len, num_kv_heads, v_head_dim).contiguous(),
        )
    torch.cuda.synchronize()
    o = o.reshape(batch_size * qo_len, num_heads * v_head_dim)
    fa3_outputs.append(o)
    return fa3_outputs
...
def run_flash_infer(
    ...
    kv_indptr = torch.arange(0, batch_size + 1, device="cuda:0", dtype=torch.int32) * kv_cache_len
    wrapper.plan(
        q_indptr,
        kv_indptr,
        num_heads,
...
def run_trtllm_attention(q_at_layer, kv_at_layer, kv_b_proj, s, batch_size, qo_len, kv_len):
    from tensorrt_llm._torch.attention_backend import TrtllmAttention

    trtllm_outputs, trtllm_qkvs, metadata = produce_outputs(
        TrtllmAttention,
        q_at_layer,
        kv_at_layer,
        kv_b_proj,
    ...
    torch.cuda.synchronize()
    return trtllm_outputs, trtllm_qkvs
...
def test_qwen_prefill_attention_backend(s: Scenario):
    ...
    # run trtllmAttention
    trtllm_outputs, trtllm_qkvs = run_trtllm_attention(
        q_at_layer, kv_at_layer, kv_b_proj, s, batch_size, qo_len, kv_len
    )

    qkvs = trtllm_qkvs

    # run flash_attention_v2
    fa2_outputs = run_flash_attention_v2(
        qkvs, batch_size, qo_len, kv_cache_len, num_heads, head_dim, num_kv_heads, v_head_dim, dtype
    )

    # run flash_attention_v3
    fa3_outputs = run_flash_attention_v3(
        qkvs, batch_size, qo_len, kv_cache_len, num_heads, head_dim, num_kv_heads, v_head_dim, dtype
    )

    # run flash_infer
    flashinfer_outputs = run_flash_infer(
        qkvs, batch_size, qo_len, kv_cache_len, num_heads, head_dim, num_kv_heads, v_head_dim, dtype
    )
