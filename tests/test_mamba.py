...
def selective_state_update_ref(state,
  ...
  B = repeat(
      B, "b g n -> b (g h) n", h=nheads // num_group)  # (batch, nheads, dstate)
  C = repeat(
      C, "b g n -> b (g h) n", h=nheads // num_group)  # (batch, nheads, dstate)
...
def reference_torch_mamba_selective_scan_update(ssm_states, x, B, C, z, dt, A,
                                                D, dt_bias, indices, num_group):
  ...
  copy_ssm_states[
      indices] = local_ssm_states  # .permute(0, 1, 3, 2).contiguous()
...
def test_mamba_selective_scan_update(batch_size, nheads, num_group, dstate,
                                     head_dim):
  num_max_batch = 256
  num_batch = batch_size
  ssm_states = torch.rand((num_max_batch, nheads, head_dim, dstate),
  ...
  my_out = hpc.selective_state_update(my_ssm_states, zxbcdt, AD, dt_bias,
                                      indices, num_group)
  ...
  assert gt.dtype == my.dtype
  assert gt.shape == my.shape
  assert torch.allclose(my.to(torch.float), gt.to(torch.float), atol=0.5)
