...
class MultiNodeHandle:
    def __init__(
        ...
        super().__init__()
        # init
        self.rank_ = multinodecomm.GetRank()
        self.world_size_ = multinodecomm.GetWorldSize()

        # inits sub-team
        self.sub_team_ = sub_team
        ...
        def get_signal_size():
            ...
            # channels. Each signal is 64 bits, which is the minimum unit for atomic cas.
            SIGNAL_SIZE = MAX_CUDA_P2P_DOMAIN_SIZE * max_num_blocks * 8
            return SIGNAL_SIZE
        ...
        # create buffer
        self.org_buffer_dict_ = multinodecomm.CreateTensorSync(total_size, sub_team)
        ...
        self.multinode_data_buffer_ = self.org_buffer_dict_[-2][: self.buffer_size_]
        self.subteam_multimem_data_buffer_ = self.org_buffer_dict_[-3][: self.buffer_size_]
        ...
        self.multinode_signal_buffer_ = self.org_buffer_dict_[-2][signal_offset:]
        self.subteam_multimem_signal_buffer_ = self.org_buffer_dict_[-3][signal_offset:]
    ...
    def get_subteam_multimem_buff(
        ...
        return (
            self.subteam_multimem_data_buffer_[storage_offset : storage_offset + ask_size]
            .view(dtype)
            .reshape(sizes)
        )

    def get_subteam_multimem_signal(self, *sizes: Any, storage_offset: int = 0) -> torch.Tensor:
        # force signal buffer use int64
        dtype = torch.int64
        ...
        return (
            self.subteam_multimem_signal_buffer_[storage_offset : storage_offset + ask_size]
            .view(dtype)
            .reshape(sizes)
        )

    def get_multinode_buff(
        self, *sizes: Any, dtype: Optional[torch.dtype] = None, storage_offset: int = 0
    ) -> torch.Tensor:
        if len(sizes) == 1 and isinstance(sizes[0], Sequence):
            sizes = tuple(sizes[0])
        ...
        return (
            self.multinode_data_buffer_[storage_offset : storage_offset + ask_size]
            .view(dtype)
            .reshape(sizes)
        )

    def get_multinode_signal(self, *sizes: Any, storage_offset: int = 0) -> torch.Tensor:
        # force signal buffer use int64
        dtype = torch.int64
        ...
        return (
            self.multinode_signal_buffer_[storage_offset : storage_offset + ask_size]
            .view(dtype)
            .reshape(sizes)
        )

    def get_multinode_signal(self) -> torch.Tensor:
        return self.multinode_signal_buffer_.view(torch.int64)
