...
def act_mul_and_blockwise_quant(gate_up: Tensor) -> Tensor:
    """Applies activation, multiplication, and quantization to the gate_up projection.
    ...
    Returns:
      ...
      Scales output tensor. scale = max(abs(x)) / fp8_max
          Shape: [N, C / 128]
          Dtype: fp32
          Attention: Scale is N major
    """
    return torch.ops.hpc.act_mul_and_blockwise_quant(gate_up)
...
@torch.library.register_fake("hpc::act_mul_and_blockwise_quant")
def act_mul_and_blockwise_quant_fake(input):
    return {
        torch.empty(
            input.shape[0], input.shape[1] // 2, dtype=torch.float8_e4m3fn, device=input.device
        ),
        torch.empty(
            intput.shape[0], input.shape[1] // 2 // 128, dtype=torch.float32, device=input.device
        ),
    }
