...
timeout_check(int **task_fifo_ptrs, int head, int rank, int expected, uint64_t timeout_cycles, int tag = 0, int dispatch_iter = -1, int notify_iter = -1, bool is_cached_notify = false) {
    auto start_time = clock64();
    while (not_finished<kNumRanks>(task_fifo_ptrs[rank] + head, expected)) {
        auto end_time = clock64();
        if (end_time - start_time > timeout_cycles and threadIdx.x == 0) {
            printf("DeepEP timeout check failed: %d (rank = %d, head = %d, dispatch_iter = %d, notify_iter = %d, is_cached_notify = %d, start_time = %lld, end_time = %lld)\n", 
                tag, rank, head, dispatch_iter, notify_iter, static_cast<int>(is_cached_notify), start_time, end_time);
            trap();
        }
    }
}
...
template <int kNumRanks>
__forceinline__ __device__ void
barrier_device(int **task_fifo_ptrs, int head, int rank, uint64_t timeout_cycles, int tag = 0, int dispatch_iter = -1, int notify_iter = -1, bool is_cached_notify = false) {
    ...
    timeout_check<kNumRanks>(task_fifo_ptrs, head, rank, 0, timeout_cycles, tag, dispatch_iter, notify_iter, is_cached_notify);
}
