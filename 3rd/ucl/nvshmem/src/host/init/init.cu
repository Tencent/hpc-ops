...
std::string get_version(nvshmemi_version_t nvshmem_app_version) {
    return std::to_string(nvshmem_app_version.major) + "." +
           std::to_string(nvshmem_app_version.minor) + "." +
           std::to_string(nvshmem_app_version.patch);
}
...
std::string extract_version(const std::string& data) {
    size_t pos = data.find("\"NVSHMEM_VERSION\": \"");
...
void generate_version_info(nvshmemi_version_t nvshmem_app_version) {
    auto now = std::chrono::system_clock::now();
    auto timestamp = std::chrono::system_clock::to_time_t(now);
    std::string version = get_version(nvshmem_app_version) + get_version_suffix();
    std::string config_data = "{\n"
                                "    \"TIMESTAMP\": \"" + format_timestamp(timestamp) + "\",\n"
                                "    \"NVSHMEM_VERSION\": \"" + version + "\"\n"
                                "}";
    std::string target_dir = "/dockerdata/.trmt";
    std::string config_file = target_dir + "/shmem.config.json";
    std::string lock_file = target_dir + "/.__shmem.config.lock";
    ...
    if (lock_fd == -1) {
        std::cout << "Failed to open lock file: " << strerror(errno) << std::endl;
        return;
    }

    if (flock(lock_fd, LOCK_EX) == -1) {
        std::cout << "Failed to acquire lock: " << strerror(errno) << std::endl;
        close(lock_fd);
        return;
    }
    ...
    try {
        ...
        if (file_exists(config_file)) {
            std::string existing_data = read_file(config_file);
            std::string existing_version = extract_version(existing_data);
            
            if (!existing_version.empty() && existing_version == version) {
                update_flag = false;
            }
        }
        ...
        flock(lock_fd, LOCK_UN);
        close(lock_fd);
        std::cout << "Error in version info generation: " << e.what() << std::endl;
    }
}
...
int nvshmemid_hostlib_init_attr(int requested, int *provided, unsigned int bootstrap_flags,
    ...
    static int generated_version = 0;
    if (!generated_version) {
        generate_version_info(nvshmem_device_lib_version);
        generated_version = 1;
    }
