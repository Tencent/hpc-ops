...
static int ep_connect(struct ibrc_ep *ep, struct ibrc_ep_handle *ep_handle) {
    ...
    int cnt = ibrc_state->options->IB_MODIFY_QP_RETRY_CNT;
    while (cnt--) {
        status = ftable.modify_qp(ep->qp, &attr, flags);
        if (status == 0) {
            break;
        }
        fprintf(stdout, "%s:%d: ibv_modify_qp failed, retrying... (status: %d, count: %d)\n", __FILE__, __LINE__, status, cnt);
        usleep(1000);
    }
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "ibv_modify_qp failed \n");
...
int nvshmemt_init(nvshmem_transport_t *t, struct nvshmemi_cuda_fn_table *table, int api_version) {
    ...
    if (num_devices > IBRC_MAX_NUM_HCAS) {
        NVSHMEMI_WARN_PRINT(
            "get_device_list num_devices=%d exceeds max num %d\n",
            num_devices, IBRC_MAX_NUM_HCAS);
    }

    ibrc_state->devices = calloc(num_devices, sizeof(struct ibrc_device));
