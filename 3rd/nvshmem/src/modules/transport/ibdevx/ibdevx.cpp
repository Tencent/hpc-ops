...
#define IBDEVX_MAX_NUM_HCAS 128
...
int nvshmemt_init(nvshmem_transport_t *t, struct nvshmemi_cuda_fn_table *table, int api_version) {
    ...
    if (num_devices > IBDEVX_MAX_NUM_HCAS) {
        NVSHMEMI_WARN_PRINT(
            "get_device_list num_devices=%d exceeds max num %d\n",
            num_devices, IBDEVX_MAX_NUM_HCAS);
    }
    ibdevx_state->devices = calloc(num_devices, sizeof(struct ibdevx_device));
