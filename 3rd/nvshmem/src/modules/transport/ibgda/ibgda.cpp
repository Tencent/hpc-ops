...
static int ibgda_rc_init2rtr(nvshmemt_ibgda_state_t *ibgda_state, struct ibgda_ep *ep,
                             const struct ibgda_device *device, int portid,
                             struct ibgda_rc_handle *peer_ep_handle, uint16_t udp_sport) {
    int status = 0;
    ...
    if (port_attr->link_layer == IBV_LINK_LAYER_INFINIBAND) {
        ...
        DEVX_SET(qpc, qpc, primary_address_path.eth_prio, ibgda_state->options->IB_SL);
        //DEVX_SET(qpc, qpc, primary_address_path.udp_sport, ah_attr.dlid);  // original
        DEVX_SET(qpc, qpc, primary_address_path.udp_sport, udp_sport);  // modify udp src port
        DEVX_SET(qpc, qpc, primary_address_path.dscp, ibgda_state->options->IB_TRAFFIC_CLASS >> 2);
...
static int ibgda_qp_rtr2rts(nvshmemt_ibgda_state_t *ibgda_state, struct ibgda_ep *ep, const struct ibgda_device *device, int portid) {
    ...
    if (ibgda_state == NULL || ibgda_state->options == NULL) {
    ...
    DEVX_SET(qpc, qpc, primary_address_path.ack_timeout, ibgda_state->options->IB_TIMEOUT);
...
static int ibgda_create_dct(nvshmemt_ibgda_state_t *ibgda_state, struct ibgda_ep **ep_ptr,
    ...
    cnt = ibgda_state->options->IB_MODIFY_QP_RETRY_CNT;
    while (cnt--) {
        status = ftable.modify_qp(ib_qp, &ib_qp_attr,
        ...
        fprintf(stdout, "%s:%d: ibv_modify_qp init2rtr for dct failed, retrying... (status: %d, count: %d)\n", __FILE__, __LINE__, status, cnt);
        usleep(1000);
    }
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
int nvshmemt_ibgda_connect_endpoints(nvshmem_transport_t t, int *selected_dev_ids,
    ...
    for (int i = 0; i < num_selected_devs; i++) {
        ...
        for (int i = 0; i < device->dci.num_eps; ++i) {
            ...
            status = ibgda_qp_rtr2rts(ibgda_state, device->dci.eps[i], device, portid);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
        ...
        uint16_t udp_sport_group[4] = {
            static_cast<uint16_t>(ibgda_state->options->GOR_QP_SPORT_1), 
            static_cast<uint16_t>(ibgda_state->options->GOR_QP_SPORT_2), 
            static_cast<uint16_t>(ibgda_state->options->GOR_QP_SPORT_3),
            static_cast<uint16_t>(ibgda_state->options->GOR_QP_SPORT_4)};
        ...
        for (int i = 0; i < num_rc_eps; ++i) {
            ...
            status = ibgda_rc_init2rtr(ibgda_state, device->rc.eps[i], device, portid,
                                       &device->rc.peer_ep_handles[i], udp_sport_group[i % 4]);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                                  "ibgda_rc_init2rtr failed on RC #%d.", i);

            status = ibgda_qp_rtr2rts(ibgda_state, device->rc.eps[i], device, portid);
            NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
int nvshmemt_init(nvshmem_transport_t *t, struct nvshmemi_cuda_fn_table *table, int api_version) {
    ...
    if (num_devices > IBGDA_MAX_NUM_HCAS) {
        NVSHMEMI_WARN_PRINT(
            "get_device_list num_devices=%d exceeds max num %d\n",
            num_devices, IBGDA_MAX_NUM_HCAS);
    }

    ibgda_state->devices = calloc(num_devices, sizeof(struct ibgda_device));
