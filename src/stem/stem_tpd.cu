...
void launch_stem_tpd(void *mask_ptr, const void *block_logits_ptr, const void *q_seq_lens_ptr,
                     const void *kv_seq_lens_ptr, int num_batch, int num_heads, int max_Qb,
                     int max_Kb, int block_size, float alpha, int initial_blocks, int window_size,
                     float k_block_num_rate, int k_block_num_bias, cudaStream_t stream) {
  ...
  kernels::stem_tpd_kernel<kEPT, kWPR, kWPC><<<grid, block, 0, stream>>>(
      ...
      reinterpret_cast<const int *>(kv_seq_lens_ptr), alpha, block_size, initial_blocks,
      window_size, k_block_num_rate, k_block_num_bias, num_heads, max_Qb, max_Kb);
}
...
void stem_tpd_async(void *mask_ptr, const void *block_logits_ptr, const void *q_seq_lens_ptr,
  ...
  if (max_Kb <= 1024) {
    launch_stem_tpd<32, 1, 8>(mask_ptr, block_logits_ptr, q_seq_lens_ptr, kv_seq_lens_ptr,
                              num_batch, num_heads, max_Qb, max_Kb, block_size, alpha,
                              initial_blocks, window_size, k_block_num_rate, k_block_num_bias,
                              stream);
  } else if (max_Kb <= 2048) {
