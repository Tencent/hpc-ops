...
#include "src/stem/stem_oam_gemm_dim192.h"
...
// kflat: [num_batch, num_head_kv, max_Kb, stem_stride * dim_qk]  BF16
// vbias: [num_batch, num_head_kv, max_Kb]                        FP32
std::tuple<torch::Tensor, torch::Tensor> stem_oam_prep_varlen_kv_dim192_entry(
    const torch::Tensor &k_fp8, const torch::Tensor &v_fp8, const torch::Tensor &kscale,
    const torch::Tensor &vscale, const torch::Tensor &kv_seq_lens,
    const torch::Tensor &cu_seqlens_kv, double lambda_mag, int64_t stem_block_size,
    int64_t stem_stride) {
  auto stream = at::cuda::getCurrentCUDAStream(k_fp8.get_device());

  TORCH_CHECK(k_fp8.device().is_cuda(), "k_fp8 must be CUDA tensor");
  TORCH_CHECK(v_fp8.device().is_cuda(), "v_fp8 must be CUDA tensor");
  TORCH_CHECK(kscale.device().is_cuda(), "kscale must be CUDA tensor");
  TORCH_CHECK(vscale.device().is_cuda(), "vscale must be CUDA tensor");
  TORCH_CHECK(kv_seq_lens.device().is_cuda(), "kv_seq_lens must be CUDA tensor");
  TORCH_CHECK(cu_seqlens_kv.device().is_cuda(), "cu_seqlens_kv must be CUDA tensor");
  TORCH_CHECK(kscale.numel() == 1,
              "stem_oam_prep_varlen_kv_dim192: kscale must be scalar (numel=1), got ",
              kscale.numel());
  TORCH_CHECK(vscale.numel() == 1,
              "stem_oam_prep_varlen_kv_dim192: vscale must be scalar (numel=1), got ",
              vscale.numel());

  // k_fp8: [total_kv, Hkv, Dqk], v_fp8: [total_kv, Hkv, Dv]
  int num_head_kv = k_fp8.size(1);
  int num_dim_qk = k_fp8.size(2);
  int num_dim_v = v_fp8.size(2);

  int num_batch = kv_seq_lens.size(0);

  int ldK = k_fp8.stride(0);
  int ldV = v_fp8.stride(0);
  ...
  auto kflat = torch::empty({num_batch, num_head_kv, max_num_stem_blocks, kflat_inner},
                            k_fp8.options().dtype(torch::kBFloat16));

  auto vbias = torch::empty({num_batch, num_head_kv, max_num_stem_blocks},
                            k_fp8.options().dtype(torch::kFloat32));

  auto v_norm_down = torch::empty({num_batch, num_head_kv, max_k_down_len},
                                  k_fp8.options().dtype(torch::kFloat32));

  TORCH_CHECK(num_dim_qk == 192 && num_dim_v == 128,
              "stem_oam_prep_varlen_kv_dim192: expected dim_qk=192 and dim_v=128, got dim_qk=",
              num_dim_qk, " dim_v=", num_dim_v);

  stem_oam_prep_varlen_kv_dim192_async(kflat.data_ptr(), vbias.data_ptr(), k_fp8.const_data_ptr(),
                                       v_fp8.const_data_ptr(), kscale.const_data_ptr(),
                                       vscale.const_data_ptr(), kv_seq_lens.const_data_ptr(),
                                       cu_seqlens_kv.const_data_ptr(), num_batch, num_head_kv, ldK,
                                       ldV, static_cast<float>(lambda_mag), max_num_stem_blocks,
                                       max_k_down_len, v_norm_down.data_ptr(), stream);
...
// Returns dim192 qflat [num_batch, num_head_q, max_Qb, stem_stride * dim_qk] BF16.
torch::Tensor stem_oam_prep_varlen_q_dim192_entry(const torch::Tensor &q_fp8,
  ...
  TORCH_CHECK(q_seq_lens.device().is_cuda(), "q_seq_lens must be CUDA tensor");
  TORCH_CHECK(cu_seqlens_q.device().is_cuda(), "cu_seqlens_q must be CUDA tensor");
  TORCH_CHECK(qscale.numel() == 1,
              "stem_oam_prep_varlen_q_dim192: qscale must be scalar (numel=1), got ",
              qscale.numel());
  ...
  int num_batch = q_seq_lens.size(0);
  int ldQ = q_fp8.stride(0);
  TORCH_CHECK(num_dim_qk == 192, "stem_oam_prep_varlen_q_dim192: expected dim_qk=192, got ",
              num_dim_qk);
  ...
  stem_oam_prep_varlen_q_dim192_async(
      qflat.data_ptr(), q_fp8.const_data_ptr(), qscale.const_data_ptr(),
      q_seq_lens.const_data_ptr(), cu_seqlens_q.const_data_ptr(), num_batch, num_head_q, ldQ,
      static_cast<int>(stem_block_size), static_cast<int>(stem_stride), max_num_q_blocks, stream);
...
// Returns dim192 block_logits [num_batch, num_head_q, max_Qb, max_Kb] BF16.
// All invalid positions (causal / OOB / padding) are set to -inf.
torch::Tensor stem_oam_gemm_dim192_entry(const torch::Tensor &qflat, const torch::Tensor &kflat,
                                         const torch::Tensor &vbias,
                                         const torch::Tensor &q_seq_lens,
                                         const torch::Tensor &kv_seq_lens, int64_t stem_block_size,
                                         int64_t stem_stride, bool causal) {
  auto stream = at::cuda::getCurrentCUDAStream(qflat.get_device());
  ...
  TORCH_CHECK(qflat.size(0) == kflat.size(0),
              "stem_oam_gemm_dim192: batch size mismatch between qflat and kflat");
  TORCH_CHECK(qflat.size(3) == kflat.size(3),
              "stem_oam_gemm_dim192: kFlatDim mismatch between qflat (", qflat.size(3),
              ") and kflat (", kflat.size(3), ")");
  TORCH_CHECK(num_head_q % num_head_kv == 0, "stem_oam_gemm_dim192: num_head_q (", num_head_q,
              ") must be divisible by num_head_kv (", num_head_kv, ")");
  TORCH_CHECK(
      vbias.size(0) == num_batch && vbias.size(1) == num_head_kv && vbias.size(2) == max_num_kb,
      "stem_oam_gemm_dim192: vbias shape mismatch, expected [", num_batch, ", ", num_head_kv, ", ",
      max_num_kb, "]");

  int num_dim_qk = kflat_inner / static_cast<int>(stem_stride);
  TORCH_CHECK(num_dim_qk == 192, "stem_oam_gemm_dim192: expected dim_qk=192, got ", num_dim_qk);
...
TORCH_LIBRARY_FRAGMENT(hpc, m) {
  ...
  m.def(
      "stem_oam_prep_varlen_kv_dim192(Tensor k_fp8, Tensor v_fp8, "
      "Tensor kscale, Tensor vscale, "
      "Tensor kv_seq_lens, Tensor cu_seqlens_kv, "
      "float lambda_mag, int stem_block_size, int stem_stride) -> (Tensor, Tensor)");
  m.impl("stem_oam_prep_varlen_kv_dim192", torch::kCUDA,
         &hpc::stem::stem_oam_prep_varlen_kv_dim192_entry);
  ...
  m.def(
      "stem_oam_prep_varlen_q_dim192(Tensor q_fp8, Tensor qscale, Tensor q_seq_lens, "
      "Tensor cu_seqlens_q, int stem_block_size, int stem_stride) -> Tensor");
  m.impl("stem_oam_prep_varlen_q_dim192", torch::kCUDA,
         &hpc::stem::stem_oam_prep_varlen_q_dim192_entry);
  ...
  m.def(
      "stem_oam_gemm_dim192(Tensor qflat, Tensor kflat, Tensor vbias, "
      "Tensor q_seq_lens, Tensor kv_seq_lens, "
      "int stem_block_size, int stem_stride, bool causal) -> Tensor");
  m.impl("stem_oam_gemm_dim192", torch::kCUDA, &hpc::stem::stem_oam_gemm_dim192_entry);
