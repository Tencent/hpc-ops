...
torch::Tensor gemm_bf16xfp32_entry(const torch::Tensor &x, const torch::Tensor &w_high,
  ...
  // blackwell split config
  if (n == 192) {
    if (use_splitk) {
      if (m >= 2048) {
        kTileM = 256;
        if (m < 3072) {
          // use (256, 32) tile for prefill
          kTileN = 32;
        } else {
          // use (256, 96) tile for prefill
          kTileN = 96;
        }
      } else {
        kTileM = 128;
        kTileN = 32;
        if (m <= 256) {
          // use (256, 32) tile and split 8 for decode, will have 48 tasks.
          split_k = 8;
        } else {
          // use (256, 32) tile and split 4 for decode, will have 48 tasks.
          split_k = 4;
        }
      }
    } else {
  ...
  if (split_k > 1) {
    ...
    if (split_flag.has_value()) {
      ...
      split_flag_tensor = torch::zeros({(m + kTileM - 1) / kTileM, (n + kTileN - 1) / kTileN},
