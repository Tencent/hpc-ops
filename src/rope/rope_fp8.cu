...
#pragma unroll
  ...
  // new cache block
  QType *k_cache_row_start = kcache_ptr + (int64_t)cache_block_idx * (int64_t)kcache_block_offset +
                             pos_in_block * (kNumKVHeads * kQKHeadDim);
  QType *v_cache_row_start = vcache_ptr + (int64_t)cache_block_idx * (int64_t)vcache_block_offset +
...
#pragma unroll
    ...
    if (batch_id < num_batch) {
      ...
      // KV cache layout: [cache_block_idx][pos_in_block][kv_head][head_dim]
      QType *k_cache_row_start = kcache_ptr +
                                 (int64_t)cache_block_idx * (int64_t)kcache_block_offset +
                                 pos_in_block * (kNumKVHeads * kQKHeadDim);
      QType *v_cache_row_start = vcache_ptr +
