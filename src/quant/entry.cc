...
std::tuple<torch::Tensor, torch::Tensor> per_token_group_fp8_quant_entry(const torch::Tensor& input,
                                                                         int64_t group_size,
                                                                         double quant_eps) {
  auto stream = at::cuda::getCurrentCUDAStream(input.get_device());

  TORCH_CHECK(group_size == 128, "now only support group_size == 128");
  ...
  auto running = per_token_group_fp8_quant_async(input_ptr, output_ptr, quant_scale_ptr, group_size,
                                                 quant_eps, hidden_state, batch_size, stream);
  TORCH_CHECK(running, "per_token_group_fp8_quant_async launch failed!");
...
TORCH_LIBRARY_FRAGMENT(hpc, m) {
  ...
  m.def(
      "per_token_group_fp8_quant(Tensor input, int group_size, float quant_eps) -> (Tensor, "
      "Tensor)");
  m.impl("per_token_group_fp8_quant", torch::kCUDA, &hpc::quant::per_token_group_fp8_quant_entry);
}
