...
namespace quant {
namespace kernels {
template <int kHiddenStates, int kWarpPerBatch, int kBatchPerBlock, int kGroupSize>
__global__ void per_token_group_quant(const __nv_bfloat16 *input_ptr, __nv_fp8_e4m3 *output_ptr,
                                      float *quant_scale, float fp8_e4m3_max, float fp8_e4m3_min,
                                      float quant_eps, int batch_size) {
...
bool per_token_group_quant_async(const void *input_ptr, void *output_ptr, void *quant_scale,
  ...
  if (hidden_states == 5120 && group_size == 128) {
    ...
    kernels::per_token_group_quant<kHiddenStates, kWarpPerBatch, kBatchPerBlock, kGroupSize>
        <<<grid, block, 0, stream>>>(reinterpret_cast<Tin *>(input_ptr),
                                     reinterpret_cast<Tout *>(output_ptr),
                                     reinterpret_cast<float *>(quant_scale), fp8_e4m3_max,
                                     fp8_e4m3_min, quant_eps, batch_size);
    ...
    kernels::per_token_group_quant<kHiddenStates, kWarpPerBatch, kBatchPerBlock, kGroupSize>
        <<<grid, block, 0, stream>>>(reinterpret_cast<Tin *>(input_ptr),
                                     ...
                                     reinterpret_cast<float *>(quant_scale), fp8_e4m3_max,
                                     fp8_e4m3_min, quant_eps, batch_size);
  } else if (hidden_states == 768 && group_size == 128) {
    constexpr int kHiddenStates = 768;
    ...
    kernels::per_token_group_quant<kHiddenStates, kWarpPerBatch, kBatchPerBlock, kGroupSize>
        <<<grid, block, 0, stream>>>(reinterpret_cast<Tin *>(input_ptr),
                                     ...
                                     reinterpret_cast<float *>(quant_scale), fp8_e4m3_max,
                                     fp8_e4m3_min, quant_eps, batch_size);
  } else if (hidden_states == 13824 && group_size == 128) {
    constexpr int kHiddenStates = 13824;
    constexpr int kWarpPerBatch = 4;
    constexpr int kBatchPerBlock = kWarpCount / kWarpPerBatch;
    constexpr int kGroupSize = 128;
    dim3 grid((batch_size + kBatchPerBlock - 1) / kBatchPerBlock);
    ...
    std::cout << "not supported hidden_size for per_token_group_quant_async:" << hidden_states
              << std::endl;
    return false;
  }
  return true;
