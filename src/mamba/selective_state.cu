...
template <int kNumSpTokens = 2>
__global__ void selective_state_update_with_speculative_sampling_kernel(
    __nv_bfloat16 *out_ptr, float *ssm_states_ptr, const int *indices_ptr,
    const __nv_bfloat16 *z_ptr, const __nv_bfloat16 *x_ptr, const __nv_bfloat16 *b_ptr,
    const __nv_bfloat16 *c_ptr, const __nv_bfloat16 *dt_ptr, const float *AD_ptr,
    const float *bias_ptr, int batch_stride_zxbcdt, int batch_stride_zxbcdtxnum_sp_tokens,
    int num_head, int head_dim, int state_dim, int num_sp_tokens,
    const int *num_accepted_tokens_ptr, int num_headxhead_dimxstate_dim,
    int num_headxhead_dimxstate_dimxnum_sp_tokens, int head_dimxstate_dim, int num_headxhead_dim,
    int num_headxhead_dimxnum_sp_tokens, int num_groupxstate_dim) {
  int idx = threadIdx.x;
  ...
  int islot = indices_ptr[ibatch];
  int isel = num_accepted_tokens_ptr[ibatch] - 1;

  auto *ssm_row = ssm_states_ptr + (islot * num_headxhead_dimxstate_dimxnum_sp_tokens +
                                    +isel * num_headxhead_dimxstate_dim +
                                    ihead * head_dimxstate_dim + irow * state_dim + icol * 4);

  auto *ssm_row_out = ssm_states_ptr + (islot * num_headxhead_dimxstate_dimxnum_sp_tokens +
                                        ihead * head_dimxstate_dim + irow * state_dim + icol * 4);

  auto *out_row = out_ptr + (ibatch * num_headxhead_dimxnum_sp_tokens + ihead * head_dim + irow);

  const auto *z_row =
      z_ptr + (ibatch * batch_stride_zxbcdtxnum_sp_tokens + ihead * head_dim + irow);
  const auto *x_row =
      x_ptr + (ibatch * batch_stride_zxbcdtxnum_sp_tokens + ihead * head_dim + irow);
  const auto *b_row =
      b_ptr + (ibatch * batch_stride_zxbcdtxnum_sp_tokens + igroup * state_dim + icol * 4);
  const auto *c_row =
      c_ptr + (ibatch * batch_stride_zxbcdtxnum_sp_tokens + igroup * state_dim + icol * 4);
  const auto *dt_row = dt_ptr + (ibatch * batch_stride_zxbcdtxnum_sp_tokens + ihead);
  ...
  vec_t<float, 4> next_ssm;
#pragma unroll
  for (int isp = 0; isp < kNumSpTokens; ++isp) {
    int ioffset = isp * batch_stride_zxbcdt;

    float z = __bfloat162float(*(z_row + ioffset));
    float x = __bfloat162float(*(x_row + ioffset));
    auto B = to<float>(load<__nv_bfloat162, 2>(b_row + ioffset));
    auto C = to<float>(load<__nv_bfloat162, 2>(c_row + ioffset));
    float dt = __bfloat162float(*(dt_row + ioffset));
    ...
    // output ssm
    store(ssm_row_out, next_ssm);
    ssm_row_out += num_headxhead_dimxstate_dim;
    ...
    if (icol == 0) {
      ...
      // output y
      out_row[0] = __float2bfloat16(out);
      out_row += num_headxhead_dim;
    }

    curr_ssm[0] = next_ssm[0];
    curr_ssm[1] = next_ssm[1];
    curr_ssm[2] = next_ssm[2];
    curr_ssm[3] = next_ssm[3];
  }
}
}  // namespace kernels
...
void selective_state_update_async(__nv_bfloat16 *out_ptr, float *ssm_states_ptr,
  ...
  int num_headxhead_dimxstate_dimxnum_sp_tokens = num_head * head_dim * state_dim * num_sp_tokens;
  int num_headxhead_dimxnum_sp_tokens = num_headxhead_dim * num_sp_tokens;
  ...
  int batch_stride_zxbcdtxnum_sp_tokens =
      (num_headxhead_dim * 2 + num_groupxstate_dim * 2 + num_head) * num_sp_tokens;
  ...
  } else if (num_sp_tokens == 2) {
    kernels::selective_state_update_with_speculative_sampling_kernel<2><<<grid, block, 0, stream>>>(
        out_ptr, ssm_states_ptr, indices_ptr, z_ptr, x_ptr, b_ptr, c_ptr, dt_ptr, AD_ptr, bias_ptr,
        batch_stride_zxbcdt, batch_stride_zxbcdtxnum_sp_tokens, num_head, head_dim, state_dim,
        num_sp_tokens, num_accepted_tokens_ptr, num_headxhead_dimxstate_dim,
        num_headxhead_dimxstate_dimxnum_sp_tokens, head_dimxstate_dim, num_headxhead_dim,
        num_headxhead_dimxnum_sp_tokens, num_groupxstate_dim);
  }
}
