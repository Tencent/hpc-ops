...
template <int kRatio>
void launch_warp_spec_mqa_indexer_logits_bf16(
  ...
  auto shm_size = (cosize(slayout_q) + cosize(slayout_k) + kTileN) * sizeof(Tin);
  ...
  auto kernel = kernels::mqa_indexer_logits_bf16_kernel<
      Tin, Tout, kTileM, kTileN, kTileK, kBlockSize, kStageK, kRatio, TiledMma, decltype(tma_q),
      decltype(tma_k), decltype(slayout_q), decltype(slayout_k)>;
...
void mqa_indexer_logits_bf16_async(void* y_ptr, const void* q_ptr, const void* kvcache_ptr,
  ...
  launch_warp_spec_mqa_indexer_logits_bf16<kRatio>(
