...
template <int kRatio>
void launch_warp_spec_mqa_indexer_logits_swap_bf16(
  ...
  auto tma_q = make_tma_copy(SM90_TMA_LOAD{}, Q, take<0, 2>(slayout_q));
  auto tma_k = make_tma_copy(SM90_TMA_LOAD{}, K, tma_copy_layout_k);

  using TiledMmaAtom = SM90_64x64x16_F32BF16BF16_SS<GMMA::Major::K, GMMA::Major::K>;
  using TiledMma = decltype(make_tiled_mma(TiledMmaAtom{}));

  auto shm_size = (cosize(slayout_q) + cosize(slayout_k) + kTileM * kStageQ) * sizeof(Tin) +
                  sizeof(int) * (2 * num_batch + 1);

  dim3 block(size(TiledMma{}) * kWarpGroupN + 128);
  dim3 grid(get_sm_count());
  // dim3 grid(1);

  cutlass::FastDivmod num_split_divider(num_split);

  auto kernel = kernels::mqa_indexer_logits_bf16_swap_kernel<
      Tin, Tout, kTileM, kTileN, kTileK, kWarpGroupN, kBlockSize, kStageQ, kStageK, kRatio,
      TiledMma, decltype(tma_q), decltype(tma_k), decltype(slayout_q), decltype(slayout_k)>;

  cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);
  kernel<<<grid, block, shm_size, stream>>>(
      tma_q, tma_k, reinterpret_cast<const Tin*>(w_ptr), reinterpret_cast<Tout*>(y_ptr),
      reinterpret_cast<const int*>(cu_seqlens_q_ptr), reinterpret_cast<const int*>(seqlens_kv_ptr),
      reinterpret_cast<const int*>(block_ids_ptr), num_batch, total_seq_q, num_head_q, head_dim,
      num_kvcache_blocks, num_seq_max_blocks, max_context_len, num_split_divider);
}
...
void mqa_indexer_logits_bf16_async(void* y_ptr, const void* q_ptr, const void* kvcache_ptr,
  ...
  launch_warp_spec_mqa_indexer_logits_swap_bf16<kRatio>(
