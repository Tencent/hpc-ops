...
template <int kRatio>
void launch_warp_spec_mqa_indexer_logits_swap_fp8(
  ...
  auto shm_size = (cosize(slayout_q) + cosize(slayout_k)) * sizeof(Tin) +
                  kTileM * kStageQ * sizeof(cute::bfloat16_t) + sizeof(int) * (2 * num_batch + 1);
  ...
  auto kernel = kernels::mqa_indexer_logits_swap_fp8_kernel<
...
void mqa_indexer_logits_fp8_async(void* y_ptr, const void* q_ptr, const void* kvcache_ptr,
  ...
  launch_warp_spec_mqa_indexer_logits_swap_fp8<kRatio>(
