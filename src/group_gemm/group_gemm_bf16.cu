...
void launch_group_gemm_bf16(void *y_ptr, const void *x_ptr, const void *w_ptr,
                           ...
                           int m, int n, int k, bool update_tma, bool use_pdl,
                           cudaStream_t stream) {
  ...
  if (update_tma) {
    ...
    // bf16 group gemm does not use the persistent task map; always plain
    // expert-per-block descriptor update (kAssignTask=false).
    ...
    if (use_pdl) {
      ...
      cudaLaunchKernelEx(&launch_config, kernel, td_xy, tma_xy, (const Tin *)x_ptr,
                         ...
                         (int *)tiles_ptr, (int *)cu_tiles_ptr, (int4 *)nullptr, num_group, m, n, k,
                         num_sm);
    } else {
      ...
      kernels::update_grouped_tma<Tin, Tout, decltype(tma_x), decltype(tma_y), kTileM,
          ...
          <<<num_group + 1, kThreadPerBlock, 0, stream>>>(
              ...
              (const int *)cu_seqlens_ptr, (int *)tiles_ptr, (int *)cu_tiles_ptr, (int4 *)nullptr,
              num_group, m, n, k, num_sm);
    }
  }
  ...
  // 1. group gemm
  {
    ...
    if (use_pdl) {
      ...
      if (k <= 1024 || n <= 1024) {
        ...
        auto kernel = kernels::group_gemm_bf16_kernel<decltype(config), decltype(tma_x),
