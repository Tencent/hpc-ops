    ...
    if (use_pdl) {
      ...
      if (task_map_ptr != nullptr) {
        ...
        cudaLaunchKernelEx(&launch_config, kernel, tma_w, tma_xy, (int *)seqlens_ptr,
                           (int *)tiles_ptr, (int *)cu_tiles_ptr, (int4 *)task_map_ptr, num_group, m,
                           n, k, flat_divider);
      ...
      if (task_map_ptr != nullptr) {
        ...
        kernel<<<grid, block, shm_size, stream>>>(tma_w, tma_xy, (int *)seqlens_ptr,
                                                  ...
                                                  (int4 *)task_map_ptr, num_group, m, n, k,
                                                  flat_divider);
      } else if (k <= 1024 || n <= 1024) {
        constexpr int kTaskLoopPolicy = 1;
        int shm_seq = sizeof(int) * (num_group + 1);
  ...
  constexpr int kSwizzleY = 64;
    ...
    launch_group_gemm_bf16<kTileM, kTileN, kTileK, kStage, kWarpgroupM, kWarpgroupN, kSwizzleX,
                          kSwizzleW, kSwizzleY>(y_ptr, x_ptr, w_ptr, seqlens_ptr, cu_seqlens_ptr,
                                                tmas_ptr, tiles_ptr, cu_tiles_ptr, task_map_ptr,
                                                num_waves, num_group, m, n, k, update_tma, use_pdl,
                                                stream);
