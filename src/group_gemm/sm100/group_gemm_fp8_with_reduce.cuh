...
#ifndef SRC_GROUP_GEMM_SM100_GROUP_GEMM_FP8_WITH_REDUCE_CUH_
#define SRC_GROUP_GEMM_SM100_GROUP_GEMM_FP8_WITH_REDUCE_CUH_
...
__device__ __forceinline__ void atomic_reduce_bf16(const void *gmem_ptr, void *smem_ptr,
                                                   int bytes) {
...
__global__ void __launch_bounds__(256, 1)
    group_gemm_2sm_fp8_with_reduce_kernel(const __grid_constant__ TmaW tma_w,
                                          cute::TmaDescriptor *td_xy,
                                          typename GemmConfig::Tout *y_ptr, int *seqlens_ptr,
                                          int *cu_seqlens_ptr, float *yscale_ptr, int *tiles_ptr,
                                          int *cu_tiles_ptr, int *x_row_map_ptr,
                                          float *topk_scale_row_map_ptr, int num_group, int m,
  ...
  auto slayout_y_mn =
      make_layout(make_shape(Int<kCtaTileN>{}, Int<kEpiTileM>{}, Int<kStageTMA>{}),
                  make_stride(Int<1>{}, Int<kCtaTileN>{}, Int<kCtaTileN * kEpiTileM>{}));
...
#pragma unroll
  ...
  if (iwarp == 0 && elected) {
    ...
    auto s2g_thr_copy = s2g_tiled_copy.get_slice(idx);
    auto tYg = s2g_thr_copy.partition_D(gYY);
    auto tYs = s2g_thr_copy.partition_S(sY);
...
#pragma unroll
          for (int i = 0; i < cute::size(tCr_bf162); i++) {
            ...
            auto oscale = load<float2, 1>(reduce_topk_scale + irow);
            float2 v = tCr_fp2(i);
            mul(v, v, yscales);
            mul(v, v, oscale[0]);
...
#pragma unroll
          for (int ir = 0; ir < kCopyM; ir++) {
            if (row_valid[iepi][ir]) {
              atomic_reduce_bf16(&tYg(0, 0, ir, itile_n * kMmaSM + get<0>(cluster_coord), 0) +
                                     row_offsets[iepi][ir],
...
template <typename GemmConfig, typename TmaX, typename TmaW, typename TmaY, int kTaskLoopPolicy,
          bool kUsePDL = false>
__global__ void __launch_bounds__(256, 1)
    group_gemm_1sm_fp8_with_reduce_kernel(const __grid_constant__ TmaW tma_w,
                                          cute::TmaDescriptor *td_xy,
                                          typename GemmConfig::Tout *y_ptr, int *seqlens_ptr,
                                          int *cu_seqlens_ptr, float *yscale_ptr, int *tiles_ptr,
                                          int *cu_tiles_ptr, int *x_row_map_ptr,
  ...
  auto slayout_y_mn = make_layout(make_shape(Int<kTileN>{}, Int<kEpiTileM>{}, Int<kStageTMA>{}),
                                  make_stride(Int<1>{}, Int<kTileN>{}, Int<kTileN * kEpiTileM>{}));
...
#pragma unroll
          for (int i = 0; i < cute::size(tCr_bf162); i++) {
            int irow = iepi * kEpiTileM + get<1>(tICr4t(2 * i));
            float oscale0 = topk_scale_row_map_group[irow];
            float oscale1 = topk_scale_row_map_group[irow + 1];
            float2 v = tCr_fp2(i);
            v.x *= yscale * oscale0;
            v.y *= yscale * oscale1;
            tCr_bf162(i) = __float22bfloat162_rn(v);
...
#endif  // SRC_GROUP_GEMM_SM100_GROUP_GEMM_FP8_WITH_REDUCE_CUH_
