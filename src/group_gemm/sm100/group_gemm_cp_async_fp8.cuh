...
__global__ void __launch_bounds__(256, 1)
    group_gemm_2sm_cp_async_fp8_kernel(const __grid_constant__ TmaA tma_a,
                                       cute::TmaDescriptor *td_xy, int *seqlens_ptr,
                                       float *yscale_ptr, int *tiles_ptr, int *cu_tiles_ptr,
...
__global__ void __launch_bounds__(384, 1)
    group_gemm_1sm_cp_async_fp8_kernel(const __grid_constant__ TmaA tma_a,
                                       cute::TmaDescriptor *td_xy, cute::float_e4m3_t *Aptr,
                                       cute::float_e4m3_t *Bptr, int *cu_seqlens_ptr,
  ...
  auto A = make_tensor(make_gmem_ptr(reinterpret_cast<Tin *>(Aptr)), make_shape(m, k, num_group),
                       make_stride(k, Int<1>{}, m * k));
  auto gA1 = local_tile(A, make_tile(Int<kTileM>{}, Int<kTileK>{}), make_coord(_, _, _));
...
#pragma unroll
  ...
  constexpr int kTransactionBytesA =
      // kMmaSM * sizeof(Tin) * cosize(SLayoutA{}(_, _, 0));
      kMmaSM * sizeof(Tin) * (kCtaTileM * kTileK);
...
#pragma unroll
        ...
        for (int itile_k = 0; itile_k < ntile_k; ++itile_k) {
          ...
          if (is_tma_leader) {
            copy(tma_a.with(tma_readable[istage_k]), tAg(_, itile_m, itile_k, igroup),
                 tAs(_, 0, 0, istage_k));
            set_barrier_transaction_bytes(tma_readable[istage_k], kTransactionBytesA);
          }
