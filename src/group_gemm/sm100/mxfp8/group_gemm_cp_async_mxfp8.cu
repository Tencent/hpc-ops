...
void launch_group_gemm_1sm_cp_async_mxfp8(void *y_ptr, const void *x_ptr, const void *w_ptr,
  ...
  // Group GEMM with cp.async A + inline prepack SFA.
  {
    ...
    auto kernel =
        kernels::group_gemm_1sm_cp_async_mxfp8_kernel<decltype(config), decltype(tma_b),
                                                      decltype(tma_y), decltype(tma_sfb), true>;
    ...
    cudaLaunchKernelEx(
        &launch_config, kernel, tma_b, tma_sfb, tma_ay, reinterpret_cast<const Tin *>(x_ptr),
...
void group_gemm_cp_async_mxfp8_async(void *y_ptr, const void *x_ptr, const void *w_ptr,
                                     ...
                                     const void *x_row_map_ptr, int x_num_rows, bool is_fp4) {
  ...
  // 1SM dispatch via the shared config selector (kTileN=128, kMmaSM=1). Keeping
  // this table in dispatch.cuh ensures the descriptors fuse_moe stage-1 builds
  // for the GateUp GEMM match the config this kernel actually runs.
  auto dispatch_1sm = [&](auto cfg_tag) {
    using Cfg = typename decltype(cfg_tag)::type;
    launch_group_gemm_1sm_cp_async_mxfp8<Cfg, 1>(
        y_ptr, x_ptr, w_ptr, sfx_ptr, sfw_packed_ptr, seqlens_ptr, cu_seqlens_ptr, tmas_ptr,
        tiles_ptr, cu_tiles_ptr, num_group, m, n, k, update_tma, stream, x_row_map_ptr, x_num_rows);
  };
  if (is_fp4) {
    group_gemm_1sm_mxfp8_dispatch_selector<Tin, Tout, Tsf, TinB_fp4>(kTileM_dispatch, dispatch_1sm);
  } else {
    group_gemm_1sm_mxfp8_dispatch_selector<Tin, Tout, Tsf, Tin>(kTileM_dispatch, dispatch_1sm);
  }
}
