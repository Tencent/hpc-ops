...
__global__ void __launch_bounds__(kSfLanes)
    prepack_mxfp8_sfa_with_gather_kernel(const uint8_t *__restrict__ x_scale_ptr,
                                         uint8_t *__restrict__ dst,
                                         const int *__restrict__ x_row_map,
                                         const int *__restrict__ cu_seqlens, int K_sf,
                                         int row_stride, int num_group) {
  ...
  int rows_start = cu_seqlens[igroup];
  int rows_valid = cu_seqlens[igroup + 1] - rows_start;
  ...
  // Compute dst offset from cu_seqlens
  int dst_tile_offset = 0;
  for (int i = 0; i < igroup; i++) {
    int sl = cu_seqlens[i + 1] - cu_seqlens[i];
  ...
  // Pre-fetch gather indices for the 4 rows this thread is responsible for
  const uint8_t *row_ptrs[4];
  bool row_valids[4];
#pragma unroll
  for (int q = 0; q < 4; ++q) {
    int row_in_group = rt * row_stride + row_base_in_tile + q * 32;
    if (row_in_group < rows_valid) {
      int src_row = x_row_map[rows_start + row_in_group];
      row_ptrs[q] = x_scale_ptr + src_row * K_sf;
      row_valids[q] = true;
    } else {
      row_ptrs[q] = nullptr;
      row_valids[q] = false;
