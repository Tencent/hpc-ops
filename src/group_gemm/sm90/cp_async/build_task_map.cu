...
// Fill one group's slice of task_map: [cu_tiles[igroup] .. cu_tiles[igroup] + tiles[igroup]) ×
// num_tile_n entries. Task layout (int4 per iblock): {igroup, itile_m, itile_n, 0}.
//
// Assumes `task_map` has been pre-initialized with 0xFF (i.e. igroup = -1 sentinel)
// on any iblock index not covered here. Only the densely-packed prefix
// [0, cu_tiles[num_group] * num_tile_n) is ever touched by this kernel.
//
// Grid: num_group blocks (one block per group). Block: small (e.g. 128).
...
// Fill BOTH gateup and down task_maps in one kernel launch.  Each block
// handles one expert; threads cooperate over the union of both maps'
// entries (gateup first, then down), so the per-expert (cu_tiles, tiles)
// load is shared between the two maps.  Either map pointer may be nullptr
// to skip that map.
//
// Extra tail blocks write the sentinel value (igroup = -1) into unused
// task_map entries, so callers do not need to initialize the buffers with
// cudaMemsetAsync before this launch. The tail range starts at
// cu_tiles_ptr[num_group] * num_tile_n and ends at the allocated task_map
// length.
template <bool kUsePDL>
__global__ void build_two_task_maps_kernel(int4 *gateup_task_map, int4 *down_task_map,
  ...
  int igroup = blockIdx.x;
  if (igroup < num_group) {
    int start = cu_tiles_ptr[igroup];  // shared load: per-expert prefix
    int num_tm = tiles_ptr[igroup];    // shared load: per-expert tile count

    // Walk both maps as a single flat range: [0, total_gu) → gateup,
    // [total_gu, total_gu + total_dn) → down. Threads stride over the union.
    ...
    // Tail blocks cooperatively write sentinel -1 to the unused suffix of
    // each task_map.
    // blockIdx.x ∈ [num_group, gridDim.x).
