...
// Scatter-load A tile from global to smem using cp.async + swizzle-aware
// smem addresses computed by the G2S TiledCopy partition.
//
// Two independent compile-time axes gate the emitted code:
//
//   kHasVirtualOOB = (kNumIters * kRowsPerIter > kTileM)
//       When the ThrLayout over-supplies rows for this (kTileM, kTileK)
//       combo — e.g. kTileM=8 with kTileK=64 has ThrLayout_M=32 so
//       local_row ∈ [0,32) but only [0,8) are valid; or kTileM=48+kTileK=64
//       with kNumIters=2 reaches rows 32..63 but only 0..47 are valid —
//       we must gate with `if (local_row < kTileM)`, otherwise the virtual
//       rows write into the next stage's smem region.  For "clean" combos
//       (kTileM=16/32/48/64 with matching kTileK) this is constexpr false
//       and the gate disappears.
//
//   kFullTile = (num_valid == kTileM)  [runtime]
//       True for every tile except the last one of an expert when
//       seq_len is not a multiple of kTileM.  The fast branch skips the
//       `local_row < num_valid` ternaries (global_row = shm_row_indices[],
//       src_size = 16 unconditionally); the slow branch zero-fills invalid
//       tail rows (src_size=0) as before.
