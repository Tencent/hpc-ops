...
// batch boundary).
inline int dim576_persistent_max_num_jobs(int num_batch, int num_sm) { return num_batch + num_sm; }

inline size_t dim576_persistent_task_tensor_elems(int num_batch, int num_sm) {
  size_t list_elems = static_cast<size_t>(dim576_persistent_max_num_jobs(num_batch, num_sm)) * 4;
  size_t cu_tasks_elems = static_cast<size_t>(num_sm) + 1;
  size_t cu_splits_elems = static_cast<size_t>(num_batch) + 1;
  return list_elems + cu_tasks_elems + cu_splits_elems;
}

inline size_t dim576_persistent_task_list_offset() { return 0; }
inline size_t dim576_persistent_cu_tasks_offset(int num_batch, int num_sm) {
  return static_cast<size_t>(dim576_persistent_max_num_jobs(num_batch, num_sm)) * 4;
}
inline size_t dim576_persistent_cu_splits_offset(int num_batch, int num_sm) {
  return dim576_persistent_cu_tasks_offset(num_batch, num_sm) + static_cast<size_t>(num_sm) + 1;
}

inline size_t dim576_persistent_y_partial_elems(int num_batch, int num_head_q, int v_dim,
                                                int num_sm) {
  size_t max_splits = static_cast<size_t>(num_sm) + num_batch;
  return max_splits * static_cast<size_t>(num_head_q) * static_cast<size_t>(v_dim);
}
inline size_t dim576_persistent_lse_elems(int num_batch, int num_head_q, int num_sm) {
  size_t max_splits = static_cast<size_t>(num_sm) + num_batch;
