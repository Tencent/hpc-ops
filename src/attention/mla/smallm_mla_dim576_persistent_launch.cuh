...
#ifndef SRC_ATTENTION_MLA_SMALLM_MLA_DIM576_PERSISTENT_LAUNCH_CUH_
#define SRC_ATTENTION_MLA_SMALLM_MLA_DIM576_PERSISTENT_LAUNCH_CUH_
...
void launch_dim576_persistent(void* y_ptr, const void* q_ptr, const void* kvcache_ptr,
                              ...
                              int num_batch, int total_seq_q, int num_head_q, int qk_dim, int v_dim,
  ...
  // task tensor layout (self-describing 8-int tasks, logical batch == query token):
  //   [ task_list (max_num_jobs*8) | cu_tasks (num_sm+1) | cu_splits (total_seq_q+1) ]
  ...
  if (!task_tensor_prebuilt) {
    dim576_persistent_get_scheduler_map_async(task_list, cu_tasks, cu_splits, cu_seqlens_q_ptr,
                                              num_seq_kv_ptr, num_batch, total_seq_q, num_sm,
                                              stream, splitk);
  }
  ...
  auto attn_kernel = kernels::attention_mla_dim576_persistent_kernel<
      ...
      kBlockSize, kStage, kUseSink, kNumMathWG>;
  ...
  cudaLaunchKernelEx(&attn_cfg, attn_kernel, tma_q_nope, tma_q_rope, tma_k_nope, tma_k_rope,
                     y_partial_ptr, lse_ptr, reinterpret_cast<Tin*>(y_ptr), sink_weight_ptr,
                     block_ids_ptr, cu_tasks, reinterpret_cast<int4*>(task_list), cu_splits,
                     num_batch, total_seq_q, num_head_q, qk_dim, v_dim, num_kvcache_blocks,
                     num_seq_max_blocks, ldY, one_over_dk_log2e);
...
void run_dim576_persistent(void* y_ptr, const void* q_ptr, const void* kvcache_ptr,
                           ...
                           int ldKV, float softmax_scale, cudaStream_t stream,
                           bool task_tensor_prebuilt, bool splitk) {
  launch_dim576_persistent<kTileM, /*kTileN=*/64, /*kTileNope=*/512, /*kTileRope=*/64,
                           /*kTileV=*/512, /*kBlockSize=*/64, kUseSink, kNumMathWG>(
      y_ptr, q_ptr, kvcache_ptr, y_partial_ptr, lse_ptr, task_tensor_ptr, block_ids_ptr,
      cu_seqlens_q_ptr, num_seq_kv_ptr, sink_weight_ptr, num_batch, total_seq_q, num_head_q, qk_dim,
      v_dim, num_kvcache_blocks, num_seq_max_blocks, ldY, ldQ, ldKV, softmax_scale, stream,
      task_tensor_prebuilt, splitk);
}
...
#endif  // SRC_ATTENTION_MLA_SMALLM_MLA_DIM576_PERSISTENT_LAUNCH_CUH_
