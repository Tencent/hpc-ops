...
#ifndef SRC_ATTENTION_MLA_SPARSE_MLA_KERNELS_CUH_
#define SRC_ATTENTION_MLA_SPARSE_MLA_KERNELS_CUH_
...
#pragma unroll
  for (int im = 0; im < kM; ++im) {
    ...
    warp_sum[im] += exp2f_ftz(gSink(im) * kLog2e - gMax(im));
  }
...
template <typename Tout, typename Tin, int kTileM, int kTileN, int kTileK, int kTileV,
          typename TiledMmaQK, typename TiledMmaSV, typename TmaQ, typename TmaY, typename SLayoutQ,
          typename SLayoutK, typename SLayoutP, typename SLayoutS, typename SLayoutV,
          typename SLayoutY, int kBlockSize, int kStage>
__global__ void __launch_bounds__(384, 1)
    attention_sparse_mla_with_kvcache_bf16_multistage_ws_smallm_kernel(
        const __grid_constant__ TmaQ tma_q, const __grid_constant__ TmaY tma_y,
        void *win_kvcache_ptr, const int *win_block_ids_ptr, const int *win_topk_ids_ptr,
        void *compress_kvcache_ptr, const int *compress_block_ids_ptr,
        const int *compress_topk_ids_ptr, const int *cu_seqlens_q_ptr, const float *sink_weight_ptr,
        int num_batch, int total_seq_q, int num_head_q, int head_dim, int num_win_kvcache_blocks,
        int num_compress_kvcache_blocks, int num_win_seq_max_blocks,
        int num_compress_seq_max_blocks, int num_win_max_topk, int num_compress_max_topk,
  ...
  constexpr int kProducerBarrier = kConsumers + 1;
  ...
  const int *win_block_ids = win_block_ids_ptr + ibatch * num_win_seq_max_blocks;
  const int *compress_block_ids = compress_block_ids_ptr + ibatch * num_compress_seq_max_blocks;

  const int *win_topk_ids = win_topk_ids_ptr + itoken * num_win_max_topk;
  const int *compress_topk_ids = compress_topk_ids_ptr + itoken * num_compress_max_topk;
  ...
  auto *shm_p = shm_kv + cosize(SLayoutK{});
  auto *shm_max = reinterpret_cast<float *>(shm_p + cosize(SLayoutP{}));
  int *shm_win_kvblk_ids = reinterpret_cast<int *>(shm_max + kTileM * kWarpsPerWrapGroup);
  int *shm_compress_kvblk_ids = shm_win_kvblk_ids + num_win_seq_max_blocks;

  auto *shm_y = reinterpret_cast<Tout *>(shm_data);  // Reuse All

  __shared__ int local_index[kStage][kTileN];
  __shared__ bool valid[kStage][kTileN];
  ...
  auto gWinKV =
      make_tensor(make_gmem_ptr(static_cast<Tin *>(win_kvcache_ptr)),
                  make_shape(Int<kTileN>{}, Int<kTileK>{}), make_stride(Int<kTileK>{}, Int<1>{}));

  auto gCompressKV =
      make_tensor(make_gmem_ptr(static_cast<Tin *>(compress_kvcache_ptr)),
                  make_shape(Int<kTileN>{}, Int<kTileK>{}), make_stride(Int<kTileK>{}, Int<1>{}));
  ...
  int num_tile_win_kv = (num_win_max_topk + kTileN - 1) / kTileN;
  int num_tile_compress_kv = (num_compress_max_topk + kTileN - 1) / kTileN;
...
#pragma unroll
  ...
  // Load BlockIds
  for (int i = idx; i < num_win_seq_max_blocks; i += blockDim.x) {
    shm_win_kvblk_ids[i] = win_block_ids[i];
  }
  for (int i = idx; i < num_compress_seq_max_blocks; i += blockDim.x) {
    shm_compress_kvblk_ids[i] = compress_block_ids[i];
  }
  __syncthreads();
  ...
  if (idx >= kMathThreads) {
    ...
    auto kv_g2s_tiled_copy = make_tiled_copy(
        ...
        make_layout(make_shape(Int<2>{}, Int<64>{}), make_stride(Int<1>{}, Int<2>{})),
        make_layout(make_shape(Int<1>{}, Int<8>{})));

    auto kv_g2s_thr_copy = kv_g2s_tiled_copy.get_slice(idx);
    auto IKV = make_identity_tensor(make_shape(Int<kTileN>{}, Int<kTileK>{}));
    auto tIKVgKV = kv_g2s_thr_copy.partition_S(IKV);
    auto tWinKVgKV = kv_g2s_thr_copy.partition_S(gWinKV);
    auto tCompressKVgKV = kv_g2s_thr_copy.partition_S(gCompressKV);
    auto tKVsKV = kv_g2s_thr_copy.partition_D(sK);

    constexpr int kKVCopyN = size<1>(tIKVgKV);
    constexpr int kKVCopyK = size<2>(tIKVgKV);

    // win tile
#pragma unroll 1
    for (int itile_seq_kv = 0; itile_seq_kv < num_tile_win_kv; ++itile_seq_kv) {
      wait_barrier(kv_writable[istage], phase);
#pragma unroll
      for (int i = idx; i < kTileN; i += 128) {
        int itopk = itile_seq_kv * kTileN + i;
        if (itopk < num_win_max_topk) {
          int ikv_token = win_topk_ids[itopk];
          int iblock = ikv_token / kBlockSize;
          int islot = ikv_token % kBlockSize;
          int ikv_block = shm_win_kvblk_ids[iblock];
          int ikv_actual = ikv_block * kBlockSize + islot;
          int kv_offset = (ikv_actual - i) * head_dim;

          local_index[istage][i] = kv_offset;
          valid[istage][i] = ikv_token >= 0;
        } else {
          local_index[istage][i] = 0;
          valid[istage][i] = false;
        }
      }
      syncwarpgroup(kProducerBarrier);

#pragma unroll 8
      for (int in = 0; in < kKVCopyN; in++) {
        int ikv = get<0>(tIKVgKV(0, in, 0));
        int kv_offset = local_index[istage][ikv];
        auto tKVgKV =
            make_tensor(tWinKVgKV(_, in, _).data() + kv_offset, tWinKVgKV(_, in, _).layout());
        cute::copy(kv_g2s_tiled_copy.with(valid[istage][ikv]), tKVgKV, tKVsKV(_, in, _, istage));
      }

      cpasync_barrier_arrive_noinc(reinterpret_cast<uint64_t *>(&kv_readable[istage]));
      istage++;
...
#pragma unroll 1
    for (int itile_seq_kv = 0; itile_seq_kv < num_tile_compress_kv; ++itile_seq_kv) {
      wait_barrier(kv_writable[istage], phase);
#pragma unroll
      for (int i = idx; i < kTileN; i += 128) {
        int itopk = itile_seq_kv * kTileN + i;
        if (itopk < num_compress_max_topk) {
          int ikv_token = compress_topk_ids[itopk];
          int iblock = ikv_token / kBlockSize;
          int islot = ikv_token % kBlockSize;
          int ikv_block = shm_win_kvblk_ids[iblock];
          int ikv_actual = ikv_block * kBlockSize + islot;
...
#pragma unroll 8
      for (int in = 0; in < kKVCopyN; in++) {
        ...
        auto tKVgKV = make_tensor(tCompressKVgKV(_, in, _).data() + kv_offset,
                                  tCompressKVgKV(_, in, _).layout());
        cute::copy(kv_g2s_tiled_copy.with(valid[istage][ikv]), tKVgKV, tKVsKV(_, in, _, istage));
      }
    ...
    Tensor gSink = make_tensor<float>(Int<kM>{});
...
#pragma unroll
    for (int im = 0; im < kM; im++) {
      ...
      gSink(im) = sink_weight_ptr[ihead];
    }
    ...
    // compute casual
#pragma unroll 1
    for (int itile_seq_kv = 0; itile_seq_kv < num_tile_win_kv + num_tile_compress_kv;
         ++itile_seq_kv) {
...
#pragma unroll
        for (int im = 0; im < kM; ++im) {
          int ikv = get<0>(tI_nm(in, im));

          if (!valid[istage_read][ikv]) {
...
#pragma unroll
    ...
    final_online_softmax<kConsumers, kTileM, kM>(tYr_nm, gSum, gMax, gSink, shm_max, iwarpgroup,
                                                 iwarp_in_warpgroup, ilane_in_warpgroup);
    // to bfloat16
...
#endif  // SRC_ATTENTION_MLA_SPARSE_MLA_KERNELS_CUH_
