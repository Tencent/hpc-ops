...
#ifndef SRC_ATTENTION_DECODE_SM90_DYNAMIC_SMALLM_COMBINE_DYNAMIC_KERNELS_CUH_
#define SRC_ATTENTION_DECODE_SM90_DYNAMIC_SMALLM_COMBINE_DYNAMIC_KERNELS_CUH_
...
__global__ void attention_decode_smallm_combine_dynamic_kernel(
  ...
  int num_batch_slots = task_map_ptr[2];  // max_num_batch baked into workspace
  constexpr int kTaskStride = kSM90DynamicTaskStride;
  int num_chunks = task_map_ptr[(num_tiles_per_sm * num_sm_count + 1) * kTaskStride +
                                ihead_kv * num_batch_slots + ibatch];
...
#endif  // SRC_ATTENTION_DECODE_SM90_DYNAMIC_SMALLM_COMBINE_DYNAMIC_KERNELS_CUH_
