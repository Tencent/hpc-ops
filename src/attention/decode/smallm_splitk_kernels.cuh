...
template <typename T, int kTileM, int kTileV, int kSplitK, int kSplitMinLen>
__global__ void attention_decode_bf16_smallm_splitk_combine_kernel(
  ...
  const float *lse_batch = lse_ptr + ibatch * kSplitK * num_head_q + ihead;
  const float *split_input =
      split_input_ptr + ibatch * kSplitK * num_head_q * kTileV + ihead * kTileV + icol;
...
#pragma unroll
  for (int ichunk = 0; ichunk < kSplitK; ichunk++) {
