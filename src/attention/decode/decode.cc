...
bool attention_decode_bf16_async(void *y_ptr, const void *q_ptr, void *kcache_ptr, void *vcache_ptr,
  ...
  if (num_dim_qk == 128) {
    return decode::smallm_dim128_async(y_ptr, q_ptr, kcache_ptr, vcache_ptr, block_ids_ptr,
                                       num_seq_kvcache_ptr, num_batch, num_head_q, num_head_k,
                                       num_head_v, num_dim_qk, num_dim_v, num_kvcache_blocks,
                                       block_size, num_seq_max_blocks, ldY, ldQ, ldK, ldV, stream);
  } else if (num_dim_qk == 80) {
    return decode::m64_dim80_async(y_ptr, q_ptr, kcache_ptr, vcache_ptr, block_ids_ptr,
                                   num_seq_kvcache_ptr, num_batch, num_head_q, num_head_k,
                                   num_head_v, num_dim_qk, num_dim_v, num_kvcache_blocks,
                                   block_size, num_seq_max_blocks, ldY, ldQ, ldK, ldV, stream);
  }
