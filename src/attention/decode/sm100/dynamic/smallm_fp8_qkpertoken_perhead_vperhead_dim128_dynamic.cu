...
#include "src/attention/decode/sm100/dynamic/smallm_fp8_qkpertoken_perhead_vperhead_dim128_dynamic_splitk_kernels.cuh"
...
void launch_smallm_fp8_qkpertoken_perhead_vperhead_dim128_dynamic_splitk_kernel(
  ...
  dim3 combine_grid(num_head_q / kWarpCount, num_seq_q, num_batch);
  dim3 combine_block(kWarpCount * 32);

  cutlass::FastDivmod heads_per_group_divider(pad_heads_per_group);

  cudaLaunchConfig_t combine_launch_config;
  memset(&combine_launch_config, 0, sizeof(combine_launch_config));
  ...
  attribute_combine[0].val.programmaticStreamSerializationAllowed = 1;

  combine_launch_config.numAttrs = 1;
  combine_launch_config.attrs = attribute_combine;
