...
#ifndef SRC_ATTENTION_DECODE_SM100_DYNAMIC_SMALLM_FP8_QKPERTOKEN_PERHEAD_VPERHEAD_DIM128_DYNAMIC_SPLITK_KERNELS_CUH_
#define SRC_ATTENTION_DECODE_SM100_DYNAMIC_SMALLM_FP8_QKPERTOKEN_PERHEAD_VPERHEAD_DIM128_DYNAMIC_SPLITK_KERNELS_CUH_
  ...
  if (iwarp == 0 && elected) {
    ...
    task_info.valid =
        get_task_info<kTileN, kBlockSize>(task_info, task_map_ptr, task_map_num_chunks_ptr, iblock,
                                          splitk_head_kv_divider, num_total_ctas, num_seq_q);
    while (true) {
      if (!task_info.valid) {
        break;
      }
...
#pragma unroll 1
    ...
    // if (iblock == 125 && idx == 128) {
    //     printf("task_map_ptr:%d, %d\n", task_info.valid, task_map_ptr[0]);
    // }
    while (true) {
      if (!task_info.valid) {
        break;
...
#endif  // SRC_ATTENTION_DECODE_SM100_DYNAMIC_SMALLM_FP8_QKPERTOKEN_PERHEAD_VPERHEAD_DIM128_DYNAMIC_SPLITK_KERNELS_CUH_
