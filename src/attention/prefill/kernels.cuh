...
__global__ void __launch_bounds__(384, 1)
    attention_with_kvcache_prefill_bf16_warp_specialization_kernel(
        ...
        const __grid_constant__ TmaV tma_v, const int *seqlens_q_ptr,
...
#pragma unroll
  ...
  for (int i = idx; i < num_batch; i += blockDim.x) {
    ...
    shm_seqlens_qstart[i] = seqlens_kvcache_ptr[i] - seqlens_q_ptr[i];
  }
...
#pragma unroll
          for (int iblock_kv = 0; iblock_kv < kNumBlockPerTileN; iblock_kv++) {
            int iblock_true = block_ids_ibatch_ptr[itile_seq_kv * kNumBlockPerTileN + iblock_kv];
