...
void warp_spec_with_kvcache_dim128_async(void *y_ptr, const void *q_ptr, const void *kcache_ptr,
                                         const void *vcache_ptr, const void *seqlens_q_ptr,
                                         const void *cu_seqlens_q_ptr, const void *block_ids_ptr,
                                         const void *seqlens_kvcache_ptr, void *tmas_ptr,
                                         int num_batch, int total_seq_q, int max_seq_q,
                                         int num_dim_qk, int num_dim_v, int num_head_q,
                                         int num_head_kv, int num_kvcache_blocks, int block_size,
  ...
  bool status = BLOCK_SIZE_SWITCH(block_size, [&]() -> bool {
    ...
    using Config = AttentionKVCachePrefillConfig<Tin, Tout, TiledMmaQK, TiledMmaPV, 128, 64, 128,
    ...
    // 0. update tma
    {
      ...
      kernels::update_batched_tma_with_kvcache<Tin, decltype(tma_q), decltype(tma_y)>
          <<<num_batch, 32, 0, stream>>>(td_qy, tma_qy, (const Tin *)q_ptr, (const Tout *)y_ptr,
                                         ...
                                         num_batch, max_seq_q, num_dim_qk, num_dim_v, num_head_q,
                                         ldQ, ldY);
    }
    ...
    // 1. compute attention
    {
      ...
      kernel<<<grid, block, shm_size, stream>>>(
          tma_qy, tma_k, tma_v, (const int *)seqlens_q_ptr, (const int *)seqlens_kvcache_ptr,
