...
void attention_with_kvcache_prefill_bf16_hybrid_mask_async(
  ...
  if (num_dim_qk == 128 && num_dim_v == 128) {
    prefill::multi_stage_with_kvcache_dim128_hybrid_mask_async(
        y_ptr, q_ptr, kcache_ptr, vcache_ptr, cu_seqlens_q_ptr, block_ids_ptr, seqlens_kvcache_ptr,
        mm_prefix_range_ptr, max_spans, tmas_ptr, num_batch, total_seq_q, max_seq_q, num_dim_qk,
...
void attention_with_kvcache_prefill_qpertoken_perhead_kvpertensor_fp8_hybrid_mask_async(
  ...
  prefill::warp_spec_with_kvcache_qpertoken_perhead_kvpertensor_fp8_dim128_hybrid_mask_async(
      ...
      tmas_ptr, num_batch, total_seq_q, max_seq_q, max_seq_q_pad, num_dim_qk, num_dim_v, num_head_q,
...
void attention_with_kvcache_prefill_qkpertoken_perhead_vperhead_fp8_hybrid_mask_async(
  ...
  prefill::warp_spec_with_kvcache_qkpertoken_perhead_vperhead_fp8_dim128_hybrid_mask_async(
      y_ptr, q_ptr, kcache_ptr, vcache_ptr, qscale_ptr, kscale_ptr, pscale_ptr, vscale_ptr,
      cu_seqlens_q_ptr, block_ids_ptr, seqlens_kvcache_ptr, mm_prefix_range_ptr, max_spans,
      tmas_ptr, num_batch, total_seq_q, max_seq_q, max_seq_q_pad, num_dim_qk, num_dim_v, num_head_q,
      num_head_kv, num_kvcache_blocks, block_size, scale_block_size, num_seq_max_blocks, ldY, ldQ,
      ldK, ldK1, ldK2, ldV, ldV1, ldV2, ldKS, ldKS1, ldKS2, stream);
}
