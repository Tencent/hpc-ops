...
#include "src/attention/attention_prefill.cuh"
#include "src/attention/attention_prefill.h"
#include "src/utils/tma.cuh"
#include "src/utils/utils.cuh"

#define PREFILL_WS_HEAD_DIM_SWITCH(head_dim, ...)                                    \
  [&]() -> bool {                                                                    \
    if (head_dim == 80) {                                                            \
...
void attention_prefill_bf16_warp_specialization_async(
  ...
  bool status = PREFILL_WS_HEAD_DIM_SWITCH(num_dim_qk, [&]() -> bool {
    ...
    auto cpbox_y = tile_to_shape(LayoutY, make_shape(Int<kTileM / 2>{}, Int<kTileV>{}));
    ...
    using TiledMmaQK = decltype(make_tiled_mma(QKMMA, warpgroup_layout));
    using TiledMmaPV = decltype(make_tiled_mma(PVMMA, warpgroup_layout));
