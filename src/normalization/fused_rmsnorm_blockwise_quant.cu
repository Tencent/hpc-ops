...
template <int kWarpsPerRow, int kHiddenSize>
__global__ void fused_rmsnorm_blockwise_quant_kernel(__nv_bfloat16* y_bf16_ptr,
                                                     __nv_fp8_e4m3* y_fp8_ptr, float* y_scale_ptr,
...
#pragma unroll
      ...
      if (y_bf16_ptr) {
        auto* y_bf16_row_ptr = y_bf16_ptr + irow * kHiddenSize;
        store(y_bf16_row_ptr + col, to<__nv_bfloat16>(input[iter]));
      }
    }
  }
...
void fused_rmsnorm_blockwise_quant_async(void* y_bf16_ptr, void* y_fp8_ptr, void* y_scale_ptr,
  ...
  if (hidden_size == 128) {
    ...
    kernels::fused_rmsnorm_blockwise_quant_kernel<kWarpsPerRow, kHiddenSize>
        <<<grid, block, 0, stream>>>((__nv_bfloat16*)y_bf16_ptr, (__nv_fp8_e4m3*)y_fp8_ptr,
                                     (float*)y_scale_ptr, (__nv_bfloat16*)input_ptr,
