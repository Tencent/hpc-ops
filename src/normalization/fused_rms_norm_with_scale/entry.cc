...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> entry(
    torch::Tensor& input, torch::Tensor& weight, torch::Tensor& scale,
    double eps, bool is_moe) {
  ...
  fused_rms_norm_with_scale_async<__nv_bfloat16, __nv_fp8_e4m3>(
      input.data_ptr(), weight.data_ptr(), output.mutable_data_ptr(),
      output_fp32_ptr, output_scale2_ptr, scale.data_ptr(), eps, batch_size,
