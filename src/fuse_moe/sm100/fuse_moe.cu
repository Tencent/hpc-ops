...
void fuse_moe_async(void *output_ptr, const void *input_ptr, void *gate_up_input_ptr,
  ...
  count_and_gather_async(topk_ids_ptr, topk_pos_ptr, seqlens_ptr, cu_seqlens_ptr, tiles_ptr,
                         cu_tiles_ptr, num_seq, num_topk, num_expert_local, rank_ep,
                         num_seq_per_group_avg, stream, x_row_map_ptr);
  ...
  if (num_seq_per_group_avg <= 64) {
    ...
    group_gemm::group_gemm_cp_async_fp8_async(
        ...
        gate_up_scale_ptr, gate_up_tmas_ptr, tiles_ptr, cu_tiles_ptr, /*task_map=*/nullptr,
        num_gateup_waves, num_expert_local, total_num_seq, intermediate_size, hidden_size,
        num_seq_per_group_avg, true, use_pdl, stream, skip_token_copy ? x_row_map_ptr : nullptr,
        num_seq);
    ...
    group_gemm::group_gemm_cp_async_fp8_act_mul_async(
        ...
        gate_up_scale_ptr, act_and_mul_scale_ptr, gate_up_tmas_ptr, tiles_ptr, cu_tiles_ptr,
        num_expert_local, total_num_seq, intermediate_size, hidden_size, num_seq_per_group_avg,
        /*update_tma=*/true, use_pdl, stream, skip_token_copy ? x_row_map_ptr : nullptr, num_seq);
  }

  // 3. call down linear
  group_gemm::group_gemm_fp8_async(
      down_output_ptr, down_input_ptr, down_weight_ptr, seqlens_ptr, cu_seqlens_ptr, down_scale_ptr,
      down_tmas_ptr, tiles_ptr, cu_tiles_ptr, /*task_map=*/nullptr, num_down_waves,
...
void fuse_moe_groupwise_w4a8_async(void *output_ptr, const void *input_ptr, void *gate_up_input_ptr,
  ...
  // 0. call count_and_gather_async (produces topk_pos; w4a8 path doesn't use x_row_map)
  count_and_gather_async(topk_ids_ptr, topk_pos_ptr, seqlens_ptr, cu_seqlens_ptr, tiles_ptr,
                         cu_tiles_ptr, num_seq, num_topk, num_expert_local, rank_ep,
  ...
  const int *valid_row_range_ptr =
      ...
      (int *)cu_seqlens_ptr + num_expert_local;  // get last number as valid row
  if (use_hadamard) {
    hadamard::act_mul_hadamard_per_tensor_quant_async(
        (const T1 *)gate_up_output_ptr, (T2 *)down_input_ptr, (const float *)act_and_mul_scale_ptr,
        valid_row_range_ptr, total_num_seq, intermediate_size / 2, use_pdl, stream);
  } else {
    activation::act_mul_and_quant_async((T2 *)down_input_ptr, (const T1 *)gate_up_output_ptr,
                                        (const float *)act_and_mul_scale_ptr, valid_row_range_ptr,
                                        total_num_seq, intermediate_size, false, stream);
  }
