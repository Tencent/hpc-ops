...
#include "src/fuse_moe/sm90/fuse_moe.h"  // for reduce_async
...
void fuse_moe_cp_async(void *output_ptr, const void *input_ptr, void *gate_up_output_ptr,
  ...
  // Step 0: count tokens per expert and build row_indices / topk_pos.
  ...
  // Step 1: gate_up projection using scatter GEMM.
  //   x      : [num_seq,        hidden_size]      (fp8, original token order)
  //   weight : [num_expert_local, intermediate_size, hidden_size] -> treated as [N, K] per group
  //   output : [total_num_seq,  intermediate_size] (bf16, sorted by expert)
  //   intermediate_size here is 2 * actual_intermediate_size (gate + up fused)
  ...
  // Step 2: activation (silu(gate) * up) + quantize to fp8
  //   gate_up_output : [total_num_seq, intermediate_size]  bf16
  //   down_input     : [total_num_seq, intermediate_size/2] fp8
  ...
  // Step 3: down projection using multistage GEMM (tokens already sorted by expert)
  //   down_input  : [total_num_seq, intermediate_size/2] (fp8, sorted by expert)
  //   weight      : [num_expert_local, hidden_size, intermediate_size/2]
  //   down_output : [total_num_seq, hidden_size] (bf16, sorted by expert)
  ...
  // Step 4: reduce (scatter back to original token order with topk weighted sum)
