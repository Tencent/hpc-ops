"""Standalone CuteDSL backend for FP8 paged-prefill — Q per-token-per-head + KV per-tensor.

Forked from ``fp8_prefill_packed_qk_pertoken_v_perhead.py`` (the Q +
per-token-K + per-head-V variant).
This file owns its own cute kernel; it does not delegate to the base op.

Differences from the base variant
---------------------------------

* **K dequant**: a single FP32 scalar (per-tensor) instead of per-token-per-head
  values shipped as the FP8-byte tail of the KV cache. Stage-2 of this fork
  removes the sage K-scale pipeline entirely (``self.sage_k_stage`` storage,
  ``load_sage_k_scale_tile`` producer, the consumer waits in the softmax
  loop, the SMEM scratch for K-scale). The kernel applies ``k_const`` as a
  constant multiplier inside ``apply_sage_qk_scale`` instead of looking it
  up from SMEM.
* **V dequant**: a single FP32 scalar (per-tensor). The base op already
  reads ``vscale[head_kv]`` once per head_kv group in the epilogue, so
  per-tensor V costs nothing extra — we just pass a scalar and broadcast.
* **KV cache tensor shape**: stage-3 drops the ``scale_rows`` tail rows;
  ``kcache.shape == [num_pages, block_size, num_head_kv, head_dim]`` with
  no extra ``+ scale_rows`` rows. Storage savings are ``block_size /
  (block_size + scale_rows) - 1 ≈ -3%`` per cache allocation; vllm KV
  pool memory drops correspondingly.

Same as the base variant
------------------------

* Q dtype, K/V dtypes, output dtype, head_dim, block_size, causal mask,
  SM100 (Blackwell) target.
* Q-scale path: per-token-per-head FP32 ``[B, Hq, max_seqlens_q_pad]``,
  applied inside the BMM1 → softmax window via the sage Q-scale pipeline.
  We keep that pipeline intact — it is the load-bearing component for
  Q-side numerical fidelity.
* TMA + tcgen05 + persistent / static scheduler + cluster shape are all
  unchanged.

Stage 1 of this fork (this commit's starting point) is a verbatim copy of
the base kernel with names rebadged. Cute.compile must succeed and the
139-case wrapper-equivalent regression must pass before stage 2 / 3 begin
removing the sage K-scale pipeline and the cache tail rows.

Math semantic
-------------

  S[i, j] = (q_scale[b, Hq, i] · Q_fp8[i, :])
            @ (k_const · K_fp8[j, :]).T
          = k_const · q_scale[i] · (Q_fp8[i, :] @ K_fp8[j, :])
  P       = softmax_along_j(S)
  O[i, :] = P[i, :] @ (v_const · V_fp8[:, :])

``k_const`` enters softmax as a temperature scale (softmax is **not**
invariant to multiplicative scale; ``softmax(c · x) ≠ softmax(x)`` for
``c ≠ 1``). Caller is expected to pre-fold the desired temperature into
``k_const``. Per-tensor V is naturally absorbed by the per-head epilogue.
"""
...
class Fp8PagedPrefillKvPertensorConfig:
...
class Fp8PagedPrefillKvPertensorCuteDSL(BlackwellFusedMultiHeadAttentionForward):
    """CuteDSL FP8 paged-prefill runner."""

    def __init__(
        self, config: Fp8PagedPrefillKvPertensorConfig = Fp8PagedPrefillKvPertensorConfig()
    ):
        super().__init__(
    ...
    def _setup_attributes(self):
        ...
        self.q_stage = 2
        self.kv_stage = 2
        # Per-tensor K-scale path: no sage K-scale pipeline; the constant
        # is passed in as a host-side Float32 ``k_scalar``.
    ...
    def prepare_runtime_tensors(
        ...
        Stage-2 variant: K-scale is a host-side scalar, not a paged
        per-token tensor. The cache allocation no longer contains K-scale
        tail rows (caller may still allocate the legacy ``+scale_rows``
        layout for ABI back-compat; the kernel ignores those bytes).
    ...
    def apply_sage_qk_scale(
        ...
        """Apply qscale*k_scalar to the QK accumulator before mask/rowmax.

        Stage-2 KV-pertensor variant: the per-tensor K-scale is a single
        FP32 constant; we no longer load per-token K-scales from SMEM.
        Each S element is scaled by ``q_scale[row] * k_scalar``.
        """
...
def _validate_inputs(
    ...
    tensors = (q, kcache, vcache, qscale, vscale, cu_seqlens_q, block_ids, seqlens_kvcache)
    ...
    if _storage_data_ptr(kcache) != _storage_data_ptr(vcache):
        raise ValueError("kcache and vcache must be views into the same allocation")

    # Stride consistency: K and V must share the same physical layout
    # (the kernel uses a single set of strides per call). head_dim must be
    # innermost contiguous (TMA box tiling assumes the last dim is
    # stride-1). NHD / HND / cross-layer layouts are all accepted as long
    ...
    if kcache.stride()[-1] != 1 or vcache.stride()[-1] != 1:
        raise ValueError(
...
_FORWARD_CACHE_KV_PERTENSOR: dict[
    Fp8PagedPrefillKvPertensorConfig, Fp8PagedPrefillKvPertensorCuteDSL
] = {}


def _get_forward_kv_pertensor(
    config: Fp8PagedPrefillKvPertensorConfig,
) -> Fp8PagedPrefillKvPertensorCuteDSL:
    runner = _FORWARD_CACHE_KV_PERTENSOR.get(config)
    if runner is None:
        runner = Fp8PagedPrefillKvPertensorCuteDSL(config)
        _FORWARD_CACHE_KV_PERTENSOR[config] = runner
    return runner
...
def _default_p_scale_log2(num_head_q: int, device: torch.device) -> torch.Tensor:
    ...
    if cached is None:
        ...
        _DEFAULT_P_SCALE_LOG2_CACHE_KV_PERTENSOR[key] = cached
    return cached


def attention_prefill_fp8_cutedsl_q_pertoken_kv_pertensor(
    q: torch.Tensor,
    kcache: torch.Tensor,
    vcache: torch.Tensor,
    qscale: torch.Tensor,
    ...
    *,
    ...
    config: Fp8PagedPrefillKvPertensorConfig = Fp8PagedPrefillKvPertensorConfig(),
) -> torch.Tensor:
    _validate_inputs(
        q,
        kcache,
    ...
    return _get_forward_kv_pertensor(config)(
        q,
        kcache,
        vcache,
        qscale,
